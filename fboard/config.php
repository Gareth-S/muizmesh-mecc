	...
	if($config['worker'])
	{
		foreach($config['worker'] as $key => $password)
		{
			...
			<li class="list-group-item d-flex justify-content-between align-items-center">
				' .User::get_starred(User::simple_crypt($password,'d')). ' 
...
else if(Util::isGET('plugin') && User::isAdmin())
{
	...
	else
	{
		...
		<div class="input-group mb-3">
		  ...
		  <input type="text" class="form-control"  id="search" placeholder="' .$lang['search']. '">
		</div>
				    ...
				    foreach(Paginate::viewPage($plugins, $p, $nb) as $plugin)
					{
					  ...
					  <div id="plugin-' .$statut. '" class="card border-' .$statut. '" data-pluginid="'.$plugin.'" data-plugincardroot="true">
						...
						<div class="card-header">
						    ...
						    <h6 class="searchText">' .$lang[$plugin.'name']. '</h6>
						</div>
					    ...
					    <div class="card-body" style="min-height:180px;max-height:180px;overflow:auto">
						    ...
						    <p class="searchText card-text text-muted">' .$lang[$plugin.'description']. '</p>			  					  
					    </div>
...
else
{
	if(User::isAdmin()) {
		...
		if(HTMLForm::check('title') && HTMLForm::check('description',0,250) &&
			...
			Util::isPOST('lang') && flatDB::indir($_POST['lang'], LANG_DIR, '.php') && HTMLForm::check('date_format') && HTMLForm::check('mail') && HTMLForm::check('salt', 8, 80) && HTMLForm::check('footer_text', 1, 250) && HTMLForm::checkNb('ItemByPage') && HTMLForm::checkNb('nb_page_scroll_infinite') && CSRF::check($token) )
		{
		...
		else
		{
			...
			<div class="row">
			    ...
			    '</div>
			    <div class="col">'.
			    HTMLForm::select('editor', array('bbcode'=> $lang['bbcode'], 'markdown'=> $lang['markdown']), $config['editor'], '', 'editor_desc', true). '
			    </div>
			</div>
			<div class="row">
			    <div class="col">'.
			    HTMLForm::text('date_format', $config['date_format'], 'text', '', 'date_format_placeholder', 'date_format_desc'). 
			    '</div>
			    <div class="col">'.
			...
			<div class="row">
				...
				HTMLForm::text('salt', ($config['salt'] ? $config['salt'] : $token), 'text', '', '', 'salt_desc'). 
