...
class HTMLForm
{
	...
	public static function trip($name, $id)
	{
		...
		else
		{
			...
			$salt  = sha1($fallback.md5(trim((string) $last)));
		...
		if(CAPTCHA) {
			...
			$out .= '
				<div class="input-group mb-3">
				  <div class="input-group-prepend">
				    <span class="input-group-text">'
				    	.Plugin::hook('captcha'). '<img id="cap-img" src="' .HTML_BASEPATH . DS . 'lib' . DS . 'Captcha.lib.php?rand=' .rand(). '" alt="' .$lang['captcha']. '" />
				    </span>
	...
	# Vérification session user pour réaliser ou non le contrôle du captcha
	# HTMLForm::sessionTrip();
